...
#include <mpi.h>
  ...
  void FieldManager::sendInitialParts() {
    ...
    for(int i = 1; i < numberOfThreads + 1; i++) {
      ...
      result1 = MPI_Send(&width, 1, MPI_LONG_LONG, i, 1, MPI_COMM_WORLD);
      result2 = MPI_Send(&height, 1, MPI_LONG_LONG, i, 2, MPI_COMM_WORLD);
      result3 = MPI_Send(threadPartArray, height * width, MPI::BOOL, i, 3, MPI_COMM_WORLD);
  ...
  void FieldManager::gatherField() {
    ...
    for(int i = 1; i < numberOfThreads + 1;i++) {
      ...
      MPI_Recv(threadPart,rowHeight*chunkWidth,MPI::BOOL,i,4,MPI_COMM_WORLD,&status);
